...
import com.cumpleanos.erroresbodega.services.SriServiceWrapper;
...
public class ConsumoSriController {
    ...
    private final SriServiceWrapper sriServiceWrapper;
    ...
    public String getCedRuc(@PathVariable String ced, @PathVariable String ident) {
        ...
        try {
            PersonaSri p = sriServiceWrapper.getPersona(ced, ident).get();
            if (p == null || p.getNombreCompleto() == null || p.getNombreCompleto().isEmpty()) {
                ContribuyenteSri c = sriServiceWrapper.getContribuyente(ced).get();
